...
function FetchedMegagame(props: Game) {
    ...
    return (
        <li className={`grid sm:grid-cols-12 grid-cols-5 gap-4 pb-4 ${getJSDateFromOmegaDate(props.date) > new Date() ? 'future' : 'past'}`}>
            ...
            <div className='col-span-4 sm:col-span-11'>
                <h2 className='hover:text-omega text-2xl'>
                    ...
                    {getJSDateFromOmegaDate(props.date) < currentDate ? <p className='tag'>Active Game</p> : null}
                </h2>
...
function GameListFilter(props: { onChange: GameListParams['changeFilter']; value?: Game['type']; past: boolean }) {
    return (
        <div className='pb-4'>
            <div className='flex flex-col md:flex-row'>
                <div className='flex flex-1 flex-col justify-center'>
                    <div>
                        <label className='pr-1' htmlFor='game-filter'>
                            Showing
                        </label>
                ...
                <div className='flex flex-1 flex-col justify-center'>
                    <div>
                        ...
                        <label className='pl-1' htmlFor='only-future'>
                            Show past games
                        </label>
                    </div>
                </div>
...
function GameList(props: GameListParams) {
    ...
    if (isLeft(props.gameList)) {
        return (
            <React.Fragment>
                <p className='py-2'>Looks like an error occurred while getting the list of games.</p>
                <p className='py-2'>
                    Try refreshing your browser and trying again. If that doesn&apos;t work,
                    <Link href='contact'>
                        <a>send the webmaster a message through the contact page</a>
                    </Link>
                </p>
            </React.Fragment>
        );
    ...
    return (
        <React.Fragment>
            ...
            <ul>{filteredGames.length > 0 ? filteredGames.map((value) => <FetchedMegagame key={value._id} {...value} />) : <p>No games matched your criteria - try again!</p>}</ul>
        </React.Fragment>
    );
}
...
export default function Games(props: InferGetStaticPropsType<typeof getStaticProps>) {
    ...
    return (
        <React.Fragment>
            ...
            <MainContent>
                <GameList gameList={gameList} filter={filter} changeFilter={changeFilter} past={past === undefined ? false : past} />
            </MainContent>
        </React.Fragment>
    );
}
