...
function makeSuccessResult(message: string, game: Game): Right<EditGameSuccess> {
    return MakeRight({
        result: "Success",
        message,
        game
...
async function getResult(body: unknown, id: string): Promise<ApiResult<EditGameResult>> {
    ...
    return {
        status: 400,
        body: makeFailedResult(`Failed to Edit game, please wait and try again: ${result.left}`)
    };
}
...
async function updateGame(req: NextApiRequest, res: NextApiResponse) {
    ...
    if (typeof req.query.id === 'string') {
        ...
        res.status(400).json(makeFailedResult('Did not get a single id in the query params'))
    }
}
