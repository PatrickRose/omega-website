...
export default function SingleGame({
    ...
    const initialValues: EditGameFormValues = {
        ...
        type: game.type,
        endDate: {
            year: PlayByEmailGameDecode.is(game)
                ? game.endDate.year
                : game.date.year,
            month: PlayByEmailGameDecode.is(game)
                ? game.endDate.month
                : game.date.month,
            day: PlayByEmailGameDecode.is(game)
                ? game.endDate.day
                : game.date.day
        },
        date: {
    ...
    const onSubmit = async (
        values: FormikValues,
        { setSubmitting, setErrors }: FormikHelpers<EditGameFormValues>
    ) => {
        try {
            ...
            const innerKeys: (keyof EditGameFormValues["date"])[] = [
                "day",
                "month",
                "year"
            ];
    ...
    return (
        <React.Fragment>
            <Hero>
                <HeroHeading>Edit {game.name}</HeroHeading>
            </Hero>
            <MainContent>
                <GameAdmin
                    onSubmit={onSubmit}
...
export const getStaticProps: GetStaticProps<EditGameProps> = async (
    context
) => {
