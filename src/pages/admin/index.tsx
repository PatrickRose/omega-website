...
function AdminSection({ title, sectionLinks, children }: { title: string, sectionLinks: LinkDef[], children?: ReactNode }) {
    return <React.Fragment>
        ...
        <ul className="py-2">
            {
                sectionLinks.map(
                    val => <li key={val.linkPath} className="py-1"><Link href={val.linkPath}><a>{val.linkText}</a></Link></li>
                )
            }
        </ul>
        {children}
...
export default function AdminPage() {
    ...
    return <React.Fragment>
        ...
        <MainContent>
            ...
            <AdminSection title="User administration" sectionLinks={userLinks} />
            <AdminSection title="Game administration" sectionLinks={gameLinks}>
