...
type GameAdminProps<T extends CreateGameFormValues | EditGameFormValues> = {
    onSubmit: (
        values: FormikValues,
        { setSubmitting, setErrors, setValues }: FormikHelpers<T>
    ) => Promise<void>;
    initialValues: T;
...
function FormErrorMessage({ errors, touched }: FormErrorMessageProps) {
    ...
    const keys: [
        key: keyof (CreateGameFormValues | EditGameFormValues),
        label: string
    ][] = [
        ["name", "Game name"],
        ["designer", "Designer"],
    ...
    for (let [key, label] of keys) {
        if (key == "date") {
            if (errors.date && touched.date) {
                errorDivs.push(
                    <li key={key}>
                        <strong>{label}</strong>: {errors.date.day}
                    </li>
                );
            }
        } else if (key == "endDate") {
            if (errors.endDate && touched.endDate) {
                errorDivs.push(
                    <li key={key}>
                        <strong>{label}</strong>: {errors.endDate.day}
                    </li>
                );
            }
        } else {
...
function GamePreview({
    ...
    values: CreateGameFormValues | EditGameFormValues;
}) {
                    ...
                    <div className="flex flex-col lg:flex-row">
                        <div className="flex-1 px-2">
                            <Form>
                                ...
                                <div>
                                    <p>
                                        The game description is written using{" "}
                                        <a href="https://daringfireball.net/projects/markdown/">
                                            markdown
                                        </a>
                                        . A preview is shown, but the main thing
                                        is:
                                    </p>
                                    <ul className="list-disc pl-8">
                                        <li>
                                            ...
                                            <code className="code-snippet">
                                                [Link
                                                Text](http://link/goes/here)
                                            </code>
                                        </li>
                                        <li>
                                            You can add list of items using a
                                            series of{" "}
                                            <code className="code-snippet">
                                                *
                                            </code>{" "}
                                            on a new lines, like so:
                                        ...
                                        <li>
                                            You can <em>*emphasise*</em> and{" "}
                                            <strong>**bold**</strong> things.
                                        </li>
                                    </ul>
                                </div>
                                <div>
                                ...
                                <div
                                    className={`${
                                        props.values.type === "Play-By-Email"
                                            ? ""
                                            : "hidden"
                                    }`}
                                >
...
Using getUTC etc gives us { year:${jsDate.getUTCFullYear()}, month: ${
...
export function validate(values: FormikValues) {
    ...
    if (values.type == "Play-By-Email") {
        ...
        if (
            !errors.date &&
            !errors.endDate &&
            formValuesToJSDate(values, "endDate") <
                formValuesToJSDate(values, "date")
        ) {
            errors.endDate = {
                day: "The end date must be after the start date"
            };
        }
    }
